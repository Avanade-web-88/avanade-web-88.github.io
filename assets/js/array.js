...
const ultimo = newCarros.pop(); //retorna o elemento removido do final

const primeiro = newCarros.shift(); //retorna o elemento removido do começo

const nomeCompleto = "Daniel Kotsugai Sarmento";
...
console.log(primeiras, "slice - a partir da primeira posição até a terceira que no caso é o índice 2");
...
const frutaMap = frutas.map(filtro); // map - retorna um novo array. Do início ao fim.
const frutaFilter = frutas.filter(filtro); // filter - retorna um novo array. Filtrando apenas true.
const frutaFind = frutas.find(filtro); // find - retorna a primeira ocorrência que for true.
console.log(frutaMap);
console.log(frutaFilter);
console.log(frutaFind);
...
const ages = [ 25,21,34,54,60,54,87,10,5,4,16,18,28,34];

const agesFilter = ages.filter((ages) => ages >= 1 && ages <= 60);

console.log(agesFilter,"adulto");

const agesMap = ages.map((ages) => {
    if (ages >= 0&& ages <= 2){
        return "bebê";
    } else if (ages >= 3 && ages <= 11){
        return "criança";
    } else if (ages >= 18 && ages <= 17){
        return "adolescente";
...
const agesFind = ages.find((ages) => ages >= 60);
